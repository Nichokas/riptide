...
impl KeybindGroup {
    ...
    pub fn filtered_groups(query: &str) -> Vec<FilteredKeybindGroup> {
        ...
        for g in Self::all() {
            ...
            let binds: Vec<&'static Keybind> = g
                ...
                .collect();
            if !binds.is_empty() {
                out.push(FilteredKeybindGroup { title: g.title, binds });
            }
        }
        out
    }
    ...
    pub fn filtered_bind_count(query: &str) -> usize {
        Self::filtered_groups(query).iter().map(|g| g.binds.len()).sum()
    }
...
mod tests {
    ...
    fn help_filter_empty_returns_all() {
        ...
        assert_eq!(all.len(), 5);
        assert_eq!(KeybindGroup::filtered_bind_count(""), KeybindGroup::total_bind_count());
    }
    ...
    fn help_filter_matches_action_case_insensitive() {
        ...
        assert_eq!(count, 1);
        assert!(groups.iter().any(|g| g.binds.iter().any(|b| b.action.to_lowercase().contains("shuffle"))));
        // Case insensitive
    ...
    fn help_filter_matches_key() {
        ...
        let has_tab = groups.iter().flat_map(|g| &g.binds).any(|b| b.key.contains("Tab"));
        assert!(has_tab);
    }
    ...
    fn help_filter_trims_whitespace() {
        ...
        assert_eq!(KeybindGroup::filtered_bind_count("  shuffle  "), KeybindGroup::filtered_bind_count("shuffle"));
    }
}
